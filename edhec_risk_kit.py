...
def compound(r):
    ...
    Utilizes the additive property of logarithms
    """
...
def sharpe_ratio(r, riskfree_rate, periods_per_year):
    ...
    """
    # converts the annual risk-free rate to per period
...
# A value of 0.066, will indicate that we could observe values as large or larger than our critical value with a probability of 6.6% if the null hypothesis were true.
# p-value less than alpha means deviation from the null hypothesis is statistically significant
...
# (arg, ) is a tuple in Python. It is a tuple with a single element.
# The comma after arg is necessary to differentiate it as a tuple.
def minimize_vol(target_return, er, cov):
    """
    Takes a set of expected returns and a covariance matrix
    Returns the optimal weights that achieve the target return
    """
    n = er.shape[0] # number of assets same as number of rows in return matrix (Nx1)
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1.0),) * n # tuple of tuples of lower and upper bounds of every weight
...
def msr(riskfree_rate, er, cov):
    """
    Takes the riskfree rate, expected returns, and a covariance matrix
...
# Assuming that all returns are the same, the optimizer cannot improve the Sharpe ratio through raising returns.
# It must do so by lowering volatility.
# Since risk-free rate is 0 and er is a vector of ones, the numerator (r - risk-free rate) becomes a constant value for any weight configuration.
# Hence, minimizing the negative Sharpe ratio translates to minimizing the portfolio volatility.
def gmv(cov):
    """
    Takes a covariance matrix
